...
Nixus.prototype.accessControl = function() {
  if(this.config.acl && this.config.acl.whitelist_ip && this.config.acl.whitelist_ip.length > 0) {
    // convert IP list to a hash lookup for faster access
    var whitelist = {};
    this.config.acl.whitelist_ip.forEach(function(ip) {
      whitelist[ip] = true;
    });

    return function(req, res, next) {
      // TODO: regex-based matching of IP addresses
      if(whitelist[req.connection.remoteAddress]) {
        next();
      } else {
        res.writeHead(403);
